...
- 形式: `<type>: <簡潔な説明>`
- type: feat（新記事）, review（レビュー改善）, infra（インフラ）, fix（修正）, docs（ドキュメント）
...
- hugo ビルドエラー: フロントマターの YAML 構文を確認（インデントや引用符）
- relref エラー: パスは content/ からの相対パスで、拡張子なし
- oxfmt 差分: `npx oxfmt --write .` をコミット前に必ず実行
